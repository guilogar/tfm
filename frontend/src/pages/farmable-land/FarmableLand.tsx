...
const FarmableLand: React.FC = () => {
  ...
  return (
    <IonPage>
      {create && (
        <Redirect
          to="/dashboard/page/FarmableLand/create"
          push={true}
          exact={true}
        />
      )}
      {update && (
        <Redirect
          to={`/dashboard/page/FarmableLand/${farmableLandId}/update`}
          push={true}
          exact={true}
        />
      )}
            ...
            <IonCard key={index}>
              ...
              <IonCardHeader>
                ...
                <IonCardSubtitle>
                  ...
                  {t(`FARMABLE_LAND_TYPE_${farmableLand.type.toUpperCase()}`)},
                  {` `}
              ...
              <IonCardContent>
                <IonItem lines="none">
                  <IonLabel>{t('FARMABLE_LAND_LIST_HAVEIOT')}</IonLabel>
                  <IonCheckbox
                    checked={farmableLand.haveIOT}
                    disabled={true}
                    slot="start"
                  />
                </IonItem>
                <IonItem lines="none">
                  <IonLabel>{t('FARMABLE_LAND_IS_SQUARE')}</IonLabel>
                  <IonCheckbox
                    checked={farmableLand.isSquare}
                    disabled={true}
                    slot="start"
                  />
                </IonItem>
