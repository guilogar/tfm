...
const UpdatePhytosanitary: React.FC = (props: any) => {
  ...
  useEffect(() => {
    (async () => {
      ...
      const { data } = await api.get(
        `/cropPhytosanitary?farmId=${farmId}&cropId=${cropId}`
      );
  ...
  const handleSubmit = async (event: any) => {
    ...
    try {
      ...
      const phytosanitarys = cropsPhytosanitarys.filter(
        (cropPhytosanitarys) => {
          return (
            cropPhytosanitarys !== null && cropPhytosanitarys !== undefined
          );
        }
      );
              ...
              {phytosanitarys.map((phytosanitary, index) => {
                return (
                  <IonSelectOption value={phytosanitary} key={index}>
                    {phytosanitary.name} ({phytosanitary.price} €)
                  </IonSelectOption>
                );
              })}
            </IonSelect>
            <IonButton
              ...
              onClick={() => {
                if (phytosanitaryRef?.value)
                  setCropsPhytosanitarys([
                    ...cropsPhytosanitarys,
                    phytosanitaryRef?.value,
                  ]);
              }}
            >
